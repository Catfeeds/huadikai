...
function save()
{
	...
	if(filesize(ABSPATH.'/config/doc-config.php')>0)
	{
		...
		$tempStr = preg_replace("/'SITEMOBILEPHONE','.*?'/i","'SITEMOBILEPHONE','".$request['sitemobilephone']."'",$tempStr);
		$tempStr = preg_replace("/'SITETELEPHONE','.*?'/i","'SITETELEPHONE','".$request['sitetelephone']."'",$tempStr);
		$tempStr = preg_replace("/'SITEQQ','.*?'/i","'SITEQQ','".$request['siteqq']."'",$tempStr);
		$tempStr = preg_replace("/'SITEFAX','.*?'/i","'SITEFAX','".$request['sitefax']."'",$tempStr);
		$tempStr = preg_replace("/'SITEEMAIL','.*?'/i","'SITEEMAIL','".$request['siteemail']."'",$tempStr);
		$tempStr = preg_replace("/'SITEADDRESS','.*?'/i","'SITEADDRESS','".$request['siteaddress']."'",$tempStr);
