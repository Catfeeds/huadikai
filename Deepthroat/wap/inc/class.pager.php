...
class Pager
{
	...
	public function Show($url,$style=1)
	{
		global $request;
		if($style==0)
		{
		...
		elseif($style==1)
		{
			...
			{
				$tempStr .= $this->cPage==$i?"<span class='current'>$i</span>":'<a href="'.$this->rootpath.$url.$i.'">'.$i.$this->anchor.'</a>';
			}
			$tempStr .=	$this->nextNo()>$this->totalPageNo?'':'<a href="'.$this->rootpath.$url.$this->nextNo().$this->anchor.'">下一页</a>';
		...
		elseif($style==3)
		{
			$tpageNum=8;
			$tempStr ='';
			$tempStr .=	$this->prvNo()<1?'':'<a href="'.$this->rootpath.$url.$this->prvNo().$this->anchor.'"><img src="skins/tianxin/images/page_l.png" width="10" height="10"></a>　';
			if($this->cPage<$tpageNum/2)
			{
				$tstart=1;
				$tend=$tpageNum+1;
			...
			for($i=$tstart;$i<$tend+1;$i++)
			{
				$tempStr .= $this->cPage==$i?'<a href="'.$this->rootpath.$url.$i.'" class="now">'.$i.'</a>　':'<a href="'.$this->rootpath.$url.$i.'">'.$i.$this->anchor.'</a>　';
			}
			$tempStr .=	$this->nextNo()>$this->totalPageNo?'':'<a href="'.$this->rootpath.$url.$this->nextNo().$this->anchor.'"><img src="skins/tianxin/images/page_r.png" width="10" height="10"></a>';
			$tempStr .='';
		}
		else
		{
		$tempStr =	$this->prvNo()<1?'':'<a href="'.$this->rootpath.$url.$this->prvNo().$this->anchor.'" class="btn">上一页</a> ';
		$tempStr .=	$this->nextNo()>$this->totalPageNo?'':'<a href="'.$url.$this->nextNo().$this->anchor.'" class="btn">下一页</a>';
		}
